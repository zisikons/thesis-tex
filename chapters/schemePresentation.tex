...
%\begin{gather*}
%\begin{bmatrix} \dot{\xi(t)}  \\ \dot{\tilde{W}}_{fg} \\ \dot{\tilde{W}}_{g} \end{bmatrix}
%=
%\begin{bmatrix}
%\frac{-k}{g(x)\rho(t)} ln\bigg( \frac{\xi + 1}{\xi - 1} \bigg)
%&  \frac{1}{g(x)\rho(t)} Z_{fg}^T(x) 
%& \frac{\dot{V}(t) - \xi(t) \dot{\rho}(t)}{g(x)\rho(t)} Z_{fg}^T(x) \\
%%
%- \frac{\gamma_{fg}}{\rho(t)} Z_{fg}(x) &  \gamma_{fg} \sigma_{fg} & 0 \\
%- \frac{\gamma_{g} (\dot{V}(t) - \xi(t) \dot{\rho}(t)) }{\rho(t)} Z_{g}(x)
%& 0 & \gamma_{g} \sigma_{g}
%\end{bmatrix}
%\begin{bmatrix} \xi(t)  \\ \tilde{W}_{fg}  \\ \tilde{W}_{g} \end{bmatrix}
%+\begin{bmatrix} 
%\text{errors}
%%\epsilon_{fg}(x)
%%+ \epsilon_{g}(x)(\dot{V}(t) - \xi(t) \dot{\rho}(t))
%\\ 0  \\ 0 \end{bmatrix}
%\end{gather*}
